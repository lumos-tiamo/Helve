...
bind and dies.  The launcher passes HELVE_SERVER_NONCE and checks the health
...
def test_health_echoes_the_launch_nonce(monkeypatch) -> None:
    monkeypatch.setenv("HELVE_SERVER_NONCE", "launch-abc-123")
...
def test_health_nonce_is_null_without_a_launch_nonce(monkeypatch) -> None:
    monkeypatch.delenv("HELVE_SERVER_NONCE", raising=False)
...
def test_health_nonce_is_null_for_an_empty_env_value(monkeypatch) -> None:
    ...
    monkeypatch.setenv("HELVE_SERVER_NONCE", "")
