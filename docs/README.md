...
每个主题只有一篇正本。当前基线为 **2026-08-31 的 `main`（`a094162`）**：每条断言都能追溯到源码路径或测试，追不到的一律标为“计划中”。被取代的旧稿不再随仓库分发——它们的价值是追溯当时的取舍，而 git 历史已经保存了这一点。
...
2. **每个主题只有一篇正本。** 出现第二篇描述同一件事时，把落败的那篇移入
   git 历史，写明取代者与裁决依据，并把指向它的链接改到接替者。
3. 旧稿的决策价值由 git 历史承载。删除时在接替它的正本里写清取代关系与裁决依据，让读者能顺着 `git log --follow` 找回去——而不是在工作树里留一份不再维护的副本。
