...
| 支柱 | 用户可见结果 | 运行时支撑 |
| --- | --- | --- |
| 终端交互 | `helve` 命令、会话、命令面板、流式转录和结构化 Markdown | `shell/` 的 Ink/React 客户端 |
...
| 本地持续性 | 会话、运行状态、可观测性投影、token 统计和记忆在重启后保留 | SQLite、`~/.helve/agent/` |
...
2. **项目持续协作**：用户通过 `HELVE.md` 提供全局或项目规则；会话和三层记忆视图让已验证的偏好、近期工作和项目决定可以复用。
