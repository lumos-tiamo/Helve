...
| 环境变量 | 默认 |
|---|---|
| `HELVE_OBSERVABILITY_MAX_RUNS` | 2000 |
| `HELVE_OBSERVABILITY_MAX_AGE_DAYS` | 90 |
| `HELVE_OBSERVABILITY_MAX_BYTES` | 512 MB |
...
三个变量：`HELVE_OBSERVABILITY_MAX_RUNS`、`..._MAX_AGE_DAYS`、`..._MAX_BYTES`。
...
flowchart LR
    ...
    B --> C["诊断给出主因 + 失败节点 + 证据"]
    C --> D{"够了吗"}
    D -->|"不够"| E["读原始 trace<br/>~/.helve/agent/traces/"]
